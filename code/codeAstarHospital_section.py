...
def Pos_mapToImg (posMap, mapa, horizontal) :
    if horizontal :
        return (mapa.GetWidth() / 2) + (posMap * mapa.GetWidth() / 100)
    return mapa.GetHeight() - ((mapa.GetHeight() / 2) + (posMap * mapa.GetHeight() / 50))

def Pos_imgToMap (posImg, mapa, horizontal) :
    if horizontal :
        return ((posImg - mapa.GetWidth() / 2.0) * 100.0 / mapa.GetWidth())
    return ((posImg - mapa.GetHeight() / 2.0) * 50.0 / mapa.GetHeight())

def ConvertMtsToPxls (mts, mapa, horizontal) :
    if horizontal :
        return (mapa.GetWidth() / 2) + ((-50.0 + mts) * mapa.GetWidth() / 100)
    return mapa.GetHeight() - ((mapa.GetHeight() / 2) + ((+25.0 - mts) * mapa.GetHeight() / 50))
...
##### IMAGEM #####
def geraImagem(mapa, position, goal, c):
        ...
        while ( y < mapa.GetWidth() ):
            ...
            if (celula == -1):
                ...
                if (celula == 1):
                    ...
                    img[x, y] = 0# set the red colour onde nao se sabe o que tem, 0, 0
    ...
    grafo.setObstacles(imgEroded, mapa.GetWidth(), mapa.GetHeight())
...
Map size: (1086,443), origem: (-543,-221), resolution (m/cell) 1.000)
Position Goal in X?-44
Position Goal in Y?-8
sw: (9.360)
no mapa: ptRobot: (44.9,17.9)
na Iamgem: ptRobot: (1031.0,63.0), ptGoal: (65.0,293.0)
NUMEROS DE ESTADOS EXPANDIDOS A-Star: 975381
A* Time:  2.00703501701
QUANTIDADE DE PASSOS 1094
NO MAPA: PT_ROBOT: (44.9,17.9), PT_GOAL: (-44.0,-8.0)
NA IMAGEM: PT_ROBOT: (1031.0,63.0), PT_GOAL: (65.0,293.0)
'''
#############
'''-6,-6 >>> -5,-5
