...
def Pos_mapToImg (posMap, mapa, horizontal) :
    if horizontal :
        return (mapa.GetWidth() / 2) + (posMap * mapa.GetWidth() / 400)
    return mapa.GetHeight() - ((mapa.GetHeight() / 2) + (posMap * mapa.GetHeight() / 125))

def Pos_imgToMap (posImg, mapa, horizontal) :
    if horizontal :
        return ((posImg - mapa.GetWidth() / 2.0) * 400.0 / mapa.GetWidth())
    return ((posImg - mapa.GetHeight() / 2.0) * 125.0 / mapa.GetHeight())

def ConvertMtsToPxls (mts, mapa, horizontal) :
    if horizontal :
        return (mapa.GetWidth() / 2) + ((-200.0 + mts) * mapa.GetWidth() / 400)
    return mapa.GetHeight() - ((mapa.GetHeight() / 2) + ((+63.0 - mts) * mapa.GetHeight() / 125))
...
Position Goal in X?-175
Position Goal in Y?-45
sw: (5.256)
no mapa: ptRobot: (-28.2,33.9)
na Iamgem: ptRobot: (1338.0,272.0), ptGoal: (194.0,1024.0)
NUMEROS DE ESTADOS EXPANDIDOS A-Star: 1760576
A* Time:  3.77994918823
