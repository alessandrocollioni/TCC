...
def Pos_mapToImg (posMap, mapa, horizontal) :
    if horizontal :
        return (mapa.GetHeight() / 2) + (posMap * mapa.GetHeight() / 400)
    return mapa.GetWidth() - ((mapa.GetWidth() / 2) + (posMap * mapa.GetWidth() / 400))

def Pos_imgToMap (posImg, mapa, horizontal) :
    if horizontal :
        return ((posImg - mapa.GetHeight() / 2.0) * 400.0 / mapa.GetHeight())
    return ((posImg - mapa.GetHeight() / 2.0) * 400.0 / mapa.GetWidth())

def ConvertMtsToPxls (mts, mapa, horizontal) :
    if horizontal :
        return (mapa.GetHeight() / 2) + ((-200.0 + mts) * mapa.GetHeight() / 400)
    return mapa.GetWidth() - ((mapa.GetWidth() / 2) + ((+200.0 - mts) * mapa.GetWidth() / 400))
